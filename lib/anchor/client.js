            ...
            this.client.submitProof(req, (err, r) => {
                if (err) {
                    ...
                    if (options.awaitConfirmed) {
                        this.subscribeProof(proof_1.toAnchorProof(r), (err, p) => {
                            if (err) {
                                rej(err);
                            }
                            else {
    ...
    subscribeProof(proof, callback) {
        this.subscribeBatch((err, res) => {
            if (err) {
                ...
                this.getProof(proof.hash, proof.batchId, anchor.Anchor.Type[proof.anchorType], getProofWithReturnBatch(true))
                    .then((res) => callback(null, res))
                    .catch((err) => callback(err, proof_1.toAnchorProof(new anchor.Proof())));
            }
        }, subscribeBatchesWithFilter({
            batchId: proof.batchId,
            anchorType: anchor.Anchor.Type[proof.anchorType],
        }));
    }
