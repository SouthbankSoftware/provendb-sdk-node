...
function connect(...opts) {
    ...
    // call credentials
    const metaCallback = (params, callback) => {
        const meta = new grpc_js_1.Metadata();
        meta.add("authorization", options.credentials);
        callback(null, meta);
    };
    let callCreds = grpc_js_1.credentials.createFromMetadataGenerator(metaCallback);
    return new Client(new service.AnchorServiceClient(options.address, grpc_js_1.credentials.combineChannelCredentials(chanCred, callCreds)));
}
